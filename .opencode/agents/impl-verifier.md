---
description: Implementation verification auditor. Cross-references completed tasks against actual code. Delegates per-task checks to subagents. Use for code review against plan specs.
mode: primary
temperature: 0.1
permission:
  edit: deny
...
```
.reports/impl-verify-{phase}-{YYYY-MM-DD}.md
```

Example: `.reports/impl-verify-mvp-2026-03-05.md`
...
Format: `IF-{PHASE}-{NNN}` (e.g. `IF-MVP-001`, `IF-MVP-002`)
...
| `plan_finding_save(...)` | **Save each impl finding (IF-* IDs)** — one call per finding |
| `plan_finding_list(category:"impl")` | See existing impl findings |
| `plan_finding_get(id)` | Read one finding (IF-* ID) |
| `plan_finding_resolve(id, ...)` | Accept or reject a finding (IF-* ID) |
